...
def encontrar_columna_preferida(preferidas_exactas, patrones_fallback, excluir_patrones=None):
    excluir_patrones = excluir_patrones or []

    # 1) Primero intentamos match exacto (normalizado) para evitar elegir columnas parecidas
    for col in df.columns:
        cl = col.lower()
        if cl in preferidas_exactas:
            return col

    # 2) Luego intentamos por patrón, excluyendo columnas conflictivas
    for col in df.columns:
        cl = col.lower()
        if any(p in cl for p in excluir_patrones):
            continue
        if any(p in cl for p in patrones_fallback):
            return col
    return None
...
col_mensaje = encontrar_columna_preferida(
    preferidas_exactas=['mensaje_promocion', 'mensaje_promocion_1'],
    patrones_fallback=['mensaje_promocion', 'mensaje', 'promo'],
    excluir_patrones=[]
)
col_nro_trx = encontrar_columna_preferida(
    preferidas_exactas=['nro_trx', 'nrotrx', 'numero_trx', 'numero_transaccion'],
    patrones_fallback=['nro_trx', 'transaccion', 'trx'],
    excluir_patrones=['fecha', 'inicio', 'hora', 'date']
)
...
if col_nro_trx is None:
    for col in df.columns:
        cl = col.lower()
        if 'nro' in cl and 'fecha' not in cl and 'inicio' not in cl and 'hora' not in cl:
            col_nro_trx = col
            break
if col_mensaje is None:
    col_mensaje = encontrar_columna(['mensaje', 'promo', 'promocion'])
