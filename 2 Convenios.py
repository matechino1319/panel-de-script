...
# Identificar otras columnas importantes
...
# Verificación manual de columnas basada en el output que mostraste
if col_nro_trx is None:
    # Buscar columna que contenga 'nro' pero no 'fecha'
    for col in df.columns:
...
if col_beneficio:
    ...
    print(f"Muestra de datos: {df[col_beneficio].head(10).tolist()}")
    print(f"Tipo de datos: {df[col_beneficio].dtype}")
    print(f"Valores no nulos: {df[col_beneficio].notnull().sum()}")
    print(f"Valores únicos: {df[col_beneficio].unique()[:10]}")
...
try:
    ...
    print(f"Valores convertidos: {df_filtrado['Beneficio_limpio'].sum():.2f}")
    print(f"Valores no cero: {(df_filtrado['Beneficio_limpio'] > 0).sum()}")
